...
int main(void)
{
    ...
    unsigned char buf_to_dec[] = "6L-Z5piv5LiA5q61YmFzZTY05ZCO5bim5Yqg5Y-35ZKM5pac57q_55qE5Y-l5a2Q";

    /* test encode */
    ret = mbedtls_base64_urlsafe_encode(buf_enc_out, buf_len, &out_len, buf_to_enc, sizeof(buf_to_enc) - 1);
    if (ret != 0) {
    ...
    /* test decode */
    ret = mbedtls_base64_urlsafe_decode(buf_dec_out, buf_len, &out_len, buf_to_dec, sizeof(buf_to_dec) - 1);
    if (ret != 0) {
        printf("! mbedtls_md_setup() returned -0x%04x\n", -ret);
        goto exit;
    }
